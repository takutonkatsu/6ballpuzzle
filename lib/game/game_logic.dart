...
class WazaPatterns {
  ...
  static void init(int numRows) {
    ...
    for (var hex in allHexes) {
      var bl1 = _getN(hex, 'b', numRows); var br1 = _getN(hex, 'c', numRows);
      if (bl1 != null && br1 != null) {
        var bl2 = _getN(bl1, 'b', numRows); var br2 = _getN(bl1, 'c', numRows); var br3 = _getN(br1, 'c', numRows);
      ...
      var tl1 = _getN(hex, 'f', numRows); var tr1 = _getN(hex, 'g', numRows);
      if (tl1 != null && tr1 != null) {
        var tl2 = _getN(tl1, 'f', numRows); var tr2 = _getN(tl1, 'g', numRows); var tr3 = _getN(tr1, 'g', numRows);
  ...
  static HexCoordinate? _getN(HexCoordinate hex, String dir, int numRows) {
    int r = hex.row; int c = hex.col; bool isEven = r.isEven;
    HexCoordinate? res;
    if (dir == 'a') res = HexCoordinate(c - 1, r);
    else if (dir == 'd') res = HexCoordinate(c + 1, r);
    else if (dir == 'b') res = isEven ? HexCoordinate(c, r + 1) : HexCoordinate(c - 1, r + 1);
    else if (dir == 'c') res = isEven ? HexCoordinate(c + 1, r + 1) : HexCoordinate(c, r + 1);
    else if (dir == 'e') res = HexCoordinate(c, r + 2);
    else if (dir == 'f') res = isEven ? HexCoordinate(c, r - 1) : HexCoordinate(c - 1, r - 1);
    else if (dir == 'g') res = isEven ? HexCoordinate(c + 1, r - 1) : HexCoordinate(c, r - 1);
    if (res != null) {
...
class SimDropResult {
  ...
  SimDropResult(this.simGrid, this.newBalls, this.allMatched, {this.wazaCompleted = false, this.highestWazaMult = 0.0});
}
...
class SimGrid {
  ...
  SimGridResult? checkMatchesFrom(HexCoordinate start, BallColor color) {
     ...
     return SimGridResult(group, WazaType.none, ballsNeeded: max(0, 6 - group.length));
  }
}

double evaluateBoardLogic(SimGrid simGrid, Map<HexCoordinate, BallColor> newBalls, CPUWeights weights, {bool isEasy = false, Map<HexCoordinate, Set<BallColor>>? hintHexes}) {
  double score = 0.0;
  ...
  if (maxHeight >= 8) currentSafety *= 10.0; 
  ...
  for (var def in WazaPatterns.detailedPatterns) {
     ...
     if (colorCount == 5) baseScore = weights.reachBonus * 20.0; 
     else if (colorCount == 4) baseScore = weights.reachBonus * 5.0;
     else if (colorCount == 3) baseScore = weights.reachBonus * 1.0;
     else if (colorCount == 2) baseScore = weights.reachBonus * 0.2;
  ...
  for (int c = 0; c < 9; c++) score -= (colH[c] - colH[c+1]).abs() * 20.0 * weights.flatness;
